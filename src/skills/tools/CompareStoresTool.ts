...
// Comprehensive price database with 100+ items for UK stores (same as GetStorePricesTool)
...
/**
 * Tool to compare prices and delivery times across all UK stores
 * and determine cheapest or fastest option
 */
export default class CompareStoresTool implements LuaTool {
    name = "compare_stores";
    description = "Compare prices and delivery times across all UK stores (Sainsbury's, Tesco, ASDA, Waitrose) for given items. Returns comparison with cheapest and fastest options.";
    
    inputSchema = z.object({
        items: z.array(z.string()).describe("List of grocery items to compare"),
        preference: z.enum(["cheapest", "fastest", "both"]).optional().describe("Whether to find cheapest, fastest, or both (default: both)")
    });

    async execute(input: z.infer<typeof this.inputSchema>) {
        const { items, preference = "both" } = input;
        const stores = ["sainsbury", "tesco", "asda", "waitrose"];
        ...
        // Calculate totals for each store with individual item prices
        for (const store of stores) {
            ...
            comparisons.push({
                ...
                deliveryTimeHours: DELIVERY_TIMES[store] || 3,
                items: itemPrices
            });
        }
        ...
        if (preference === "cheapest") {
            recommendation = cheapest.store;
        } else if (preference === "fastest") {
            recommendation = fastest.store;
        } else {
            if (cheapest.store === fastest.store) {
                recommendation = `${cheapest.store} (cheapest and fastest)`;
            } else {
                recommendation = `Cheapest: ${cheapest.store} (£${cheapest.total.toFixed(2)}), Fastest: ${fastest.store} (${fastest.deliveryTimeHours} hours)`;
            }
        }
        ...
        return {
            ...
            cheapest: {
                ...
                deliveryTimeHours: cheapest.deliveryTimeHours
            },
            fastest: {
                store: fastest.store,
                total: fastest.total,
                deliveryTimeHours: fastest.deliveryTimeHours
            },
            recommendation,
            message: `Store comparison complete. ${recommendation}`
        };
    }
}
